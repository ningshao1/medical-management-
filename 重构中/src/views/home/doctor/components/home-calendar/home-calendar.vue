        ...
        <div class="control">
            ...
            <div class="checkbox-wrap">
                <el-checkbox v-model="checkAll"
                             size="large">全部任务</el-checkbox>
                <el-checkbox-group v-model="currentOptions">
                    <div v-for="option in allOptions">
                        <el-checkbox :label="option.value"
                                     :key="option.value"
                                     size="large">{{option.label}}</el-checkbox>
                    </div>
                </el-checkbox-group>
            </div>
            <div class="schedule-types">
                <div v-for="type in scheduleTypes"
                     class="schedule-type-item">
                    <div class="color-chunk"
                         :style="{backgroundColor: type.titleColor?type.titleColor:type.color}"></div>
                    <span>{{type.title}}</span>
                </div>
            </div>
        </div>
        <div class="calendar-wrap">
            <kld-calendar @date-change="dateChange"
...
<script>
import kldCalendar from "@c/kld-calendar";
...
export default {
    ...
    data() {
        return {
            ...
            maintenanceTwoLimit: true,
            badEventLimit: true,
            maintenanceOneLimit: true,
    ...
    computed: {
        ...
        allOptionsValue() {
            return this.allOptions.map(({ value }) => value);
        },
        checkAll: {
            get() {
        ...
        scheduleTypes() {
            const /*notZero = ({status}) => status !== 0,*/
                titleRender = data =>
                    `<span style="color: #666; font-size: 14px">${data}</span>`,
                contentRender = data =>
                    `<span style="color: #000; font-size: 14px">${data}</span>`,
    ...
    methods: {
        getSchedules() {
            if (this.currentOptions.length === 0) {
                ...
                return this.$axios
                    .post("/statistical/findHomePageByEngineeraAndDoctor", {
            ...
            for (const rawData of data) {
                for (const scheduleType of this.scheduleTypes) {
                    if (
                        ...
                        (typeof scheduleType.validator === "function"
                            ? scheduleType.validator(rawData)
                            : true)
                    ) {
                        schedules.push({
                            title: scheduleType.title,
                            fontColor: scheduleType.titleColor,
                            bgColor: scheduleType.color,
                            start: rawData.startTime,
                            data: rawData,
                            popupTemplate:
                                typeof scheduleType.popupRender ===
                                    "function" &&
                                scheduleType.popupRender(rawData)
                        });
                    }
                }
            }
