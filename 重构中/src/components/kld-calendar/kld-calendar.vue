    ...
    <div class="kld-calendar">
        <div class="kld-calendar-header">
            <el-button type="default"
                       size='mini'
                       @click="calendarToPrevOrNextOrToday(0)"
                       style="margin-right: 10px;">本月</el-button>
            <div class="calendar-control">
                <i shape="circle"
                   class="el-icon-arrow-left calendar"
                   @click="calendarToPrevOrNextOrToday(-1)"></i>
                <span class="calendar-current-render-date">{{currentRenderDate}}</span>
                <i class="el-icon-arrow-right calendar"
                   shape="circle"
                   icon="chevron-right"
                   @click="calendarToPrevOrNextOrToday(1)"></i>
            </div>
...
export default {
    ...
    mounted() {
        ...
        const calendar = (this.calendar = new Calendar("#calendar", {
            ...
            template: {
                allday: schedule => {
                    return `<i class="ivu-icon ivu-icon-${
                        schedule.raw.icon
                    }"></i> <span style="color:${
                        schedule.raw.fontColor
                    };font-weight:500">${schedule.title}</span>`;
                },
                monthGridHeaderExceed: count =>
                    `<span class="tui-full-calendar-weekday-grid-more-schedules">${count} 更多</span>`
            }
        }));
        ...
        if (this.useDetailPopup) {
            const kldCalendar = document.querySelector(".kld-calendar"),
                ...
                getCalendarPopupDetailsTemplate = (
                    schedule,
                    arrowDirection
                ) => `
                                        ...
                                        for (const tem of popupTemplate) {
                                            popupHtml += `
                                                ...
                                                <div class="tui-full-calendar-section-detail">
                                                    <div class="tui-full-calendar-popup-detail-item"><span class="tui-full-calendar-content">${
                                                        tem.content
                                                    }</span></div>
                                                </div>
                                                `;
            ...
            calendarPopupDetails.setAttribute(
                "class",
                "tui-full-calendar-floating-layer calendar-popup-details"
            );
            ...
            $(kldCalendar)
                .on("mouseenter", "[data-schedule-id]", ({ target }) => {
                    ...
                    if (
                        (scheduleId = target.getAttribute("data-schedule-id"))
                    ) {
                        detailPopupElement = target;
                    } else {
                        detailPopupElement = $(target).parents(
                            "[data-schedule-id]"
                        )[0];
                        scheduleId = detailPopupElement.getAttribute(
                            "data-schedule-id"
                        );
                    }
                    ...
                    if (detailPopupElement) {
                        for (const schedule of this.rawSchedules) {
                            if (scheduleId === schedule.id.toString()) {
                                ...
                                calendarPopupDetails.innerHTML = getCalendarPopupDetailsTemplate(
                                    schedule
                                );
                                ...
                                const targetClientRect = detailPopupElement.getBoundingClientRect(),
                                    ...
                                    targetOffsetRight =
                                        ...
                                        targetClientRect.right,
                                    targetOffsetTop = targetClientRect.top,
                                    targetOffsetBottom =
                                        document.documentElement.clientHeight -
                                        targetClientRect.bottom,
                                ...
                                getPopupPosition: {
                                    if (
                                        ...
                                        if (targetOffsetRight > popupWidth) {
                                            ...
                                            popupLeft =
                                                targetClientRect.right + 10;
                                            popupTop =
                                                targetOffsetTop +
                                                targetOffsetHeight / 2 -
                                                popupHeight / 2;
                                ...
                                calendarPopupDetails.style.left =
                                    popupLeft + "px";
                                calendarPopupDetails.style.top =
                                    popupTop + "px";
                                popupArrowDirection &&
                                    (calendarPopupDetails.innerHTML = getCalendarPopupDetailsTemplate(
                                        schedule,
