<template>
    <div class="inspection-task-list">
        <div v-if="addLimit" class="position-to-title">
            <Button @click="addTask" class="kld-btn" icon="plus">新增巡检任务</Button>
        </div>
        <div class="search kld-box-effect">
            <Form :model="searchData">
                <Row type="flex" :gutter="10">
                    <i-col>
                        <i-input v-model.trim="searchData.keyword" placeholder="任务名称/执行人" icon="ios-search-strong" class="kld-input"></i-input>
                    </i-col>
                    <i-col>
                        <kld-tree-select
        ...
        <div class="task-list kld-box-effect">
            <group-table :page-num="pageNum" :is-loading="isLoading" :group-datas="taskList" @on-table-click="toTaskDetails" :collapse-columns="collapseColumns"></group-table>
            <div class="task-page">
                <kld-page
...
<script type="text/jsx">
    ...
    export default {
        ...
        computed: {
            ...
            addLimit() {
                return this.checkLimit('pm:inspection:task:add');
            },
            detailsLimit() {
                return this.checkLimit('pm:inspection:task:query');
            }
        },
        methods: {
            //新增巡检任务
            addTask() {
                this.$router.push({name: 'inspection-task-add'});
            },
            ...
            getTaskList() {
                ...
                return this.$axios.post(`/inspectionSchedule/${this.user_in.osId}/list`, data)
                    .then(
                        ({data: {recordsTotal, data}}) => {
                                ...
                                taskList.push({
                                    ...
                                    tableData: {
                                        columns: [
                                            ...
                                            {render(h, {row: {startTime, endTime}, index}) {
                                                return h('p', `第${numberToLocale(index + 1)}期 (${startTime} 至 ${endTime})`);
                                            }},
                                            {render(h, {row: {executorName, executorId: infoId}}) {
                                                if (executorName) {
                                                    return h('p', ['执行人：', h(poptipShowInfo, {props: {infoId}}, [h('span', {class: 'kld-success'}, executorName)])]);
                                                } else {
                                                    return <p>执行人：无</p>;
                                                }
                                            }},
                                            {render(h, {row: {status, executeStartTime, executeEndTime}}) {
                                                if (executeEndTime) {
                                                    let dateTextClassName = 'kld-success';
                                                    return h('p', ['执行时间段：', h('span', {class: dateTextClassName}, executeStartTime), ' 至 ', h('span', {class: dateTextClassName}, executeEndTime)]);
                                                } else {
                                                    return <p>执行时间段：无</p>;
                                                }
                                            }},
                                        ...
                                        data: item.pmTasks.sort((a, b) => a.periods - b.periods) /*后台没有按期数排序*/
                                    }
                                });
                            }
