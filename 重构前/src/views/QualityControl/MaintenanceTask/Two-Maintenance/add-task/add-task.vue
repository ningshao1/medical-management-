        ...
        <Form :model="formData" :rules="routeType === 'add' ? taskFormRules : {}" ref="taskForm" label-position="top">
            ...
            <Row :gutter="20" class="mt26">
                <i-col :span="12">
                    <FormItem label="任务名称：" prop="name">
                        <i-input :disabled="routeType === 'details'" v-model.trim="formData.name" class="kld-input"
                                 placeholder="请填写任务名称"></i-input>
                    </FormItem>
                </i-col>
                <i-col :span="12">
                    <FormItem label="保养等级：">
                        <i-input value="二级保养" class="kld-input" disabled></i-input>
                    </FormItem>
                </i-col>
            </Row>
            <Row :gutter="20" class="mt26">
                <i-col :span="12">
                    <FormItem label="制定人：">
                        <poptip-show-info :info-id="routeType === 'add' ? user_in.id : formData.customId" width="100%">
                            <i-input :value="routeType === 'add' ? user_in.name : formData.customName" class="kld-input"
                                     disabled></i-input>
                        </poptip-show-info>
                    </FormItem>
                </i-col>
                <i-col :span="12">
                    <FormItem label="制定时间：">
                        <i-input v-if="routeType === 'add'" :value="moment().format('YYYY-MM-DD')" class="kld-input"
                                 disabled></i-input>
                        <i-input v-else :value="moment(formData.createTime).format('YYYY-MM-DD')" class="kld-input"
                                 disabled></i-input>
                    </FormItem>
                </i-col>
            </Row>
            <div class="mt26">
                <FormItem label="保养科室：" prop="sectionIds">
                    <div v-if="routeType === 'add'" class="inspection-section">
                        <div class="selected-section" @click="sectionModalShow = true">
                            <Tag v-for="(section, index) in sectionTreeConfirmChecked" @on-close="selectedSectionClose"
                                 :name="index" :key="section.id" type="border" closable>{{section.osName}}
                            </Tag>
                        </div>
                        <Button @click="sectionModalShow = true" class="kld-btn" icon="plus">添加科室</Button>
                    </div>
                    <template v-else>
                        <div v-for="(section, index) in sectionTreeConfirmChecked" :key="index"
                             class="inspection-section-item">{{section.title}}
                        </div>
                    </template>
                </FormItem>
            </div>
            ...
            <Row :gutter="20" class="mt26">
                <i-col :span="12">
                    <FormItem label="执行年度：" prop="syear">
                        <Select v-if="routeType === 'add'" v-model="formData.syear" class="kld-select"
                                placeholder="请选择执行年度">
                            <Option :value="currentYear + ''">{{currentYear}}年</Option>
                            <Option v-for="i in 20" :value="currentYear + i + ''" :key="i">{{currentYear + i}}年</Option>
                        </Select>
                        <i-input :value="formData.syear" v-else class="kld-input" disabled></i-input>
                    </FormItem>
                </i-col>
                <i-col :span="12">
                    <FormItem label="周期：" prop="cycle">
                        <div class="execution-cycle">
                            <Select v-if="routeType === 'add'" v-model="formData.cycle" class="kld-select"
                                    placeholder="请选择周期">
                            ...
                            <i-input :value="formData.cycle" v-else class="kld-input" disabled></i-input>
                            <span>月</span>
                        </div>
                    </FormItem>
                </i-col>
            ...
            <div v-if="formData.pmTasks.length > 0" class="inspection-plain mt26">
                <template v-if="routeType === 'add'">
                    <Row v-for="(item, index) in formData.pmTasks" :key="index" class="inspection-plain-row" type="flex"
                        ...
                        <i-col class="inspection-plain-col">
                            <div class="inspection-plain-label">
                                <span>
                                    <span class="kld-error">*</span>执行时间段：</span>
                                <DatePicker :editable="false" format="yyyy-MM-dd" :value="item.executeStartTime"
                                            @on-change="item.executeStartTime = $event" class="kld-date-picker"
                                            placeholder="开始时间"></DatePicker>
                                <span class="p0-10">至</span>
                                <DatePicker :editable="false" format="yyyy-MM-dd" :value="item.executeEndTime"
                                            @on-change="item.executeEndTime = $event" class="kld-date-picker"
                ...
                <template v-else>
                    <Row v-for="(item, index) in formData.pmTasks" :key="index" class="inspection-plain-row" type="flex"
                        ...
                        <i-col :span="3" class="inspection-plain-control">
                            ...
                            <template v-else>
                                <a href="javascript:" @click="plainEdit(item)" key="edit"
                                   v-if="moment().valueOf() < moment(item.startTime).valueOf() && ViewLimit('pm:two:task:edit')">编辑</a>
                                <a href="javascript:" @click="checkPlainDetails(item)" key="details"
                                   v-if="ViewLimit('pm:two:task:query')">查看详情</a>
                            </template>
            ...
            <div class="kld-form-footer">
                <Button v-if="routeType === 'add'" :loading="submitLoading" @click="formDataSubmit" class="kld-btn">确定
                </Button>
                <Button @click="cancelSubmit" class="kld-btn-default">取消</Button>
            </div>
        </Form>
        <Modal v-model="sectionModalShow" title="选择科室" @on-cancel="sectionModalCancel" v-if="routeType === 'add'"
               class-name="vertical-center-modal" width="auto">
...
<script type="text/jsx">
    ...
    export default {
        ...
        methods: {
            ...
            //获取保养详情
            getDetails() {
                return this.$axios.get(`/maintenanceSchedule/${this.taskId}`)
                    .then(({data}) => {
                        if (data) {
                            this.formData = data;
            ...
            plainEditConfirm() {
                if (this.validPlainsDate(this.currentPlainFormData)) {
                    ...
                    this.$axios
                        .put("/maintenanceSchedule/updateTask", {
                            id: this.currentPlainFormData.id,
                            executeStartTime: this.currentPlainFormData
                                .executeStartTime,
                            executeEndTime: this.currentPlainFormData
                                .executeEndTime,
                            executorId: this.currentPlainFormData.executorId
                        })
                        .then(() => {
                            this.$Message.success("保养计划编辑成功");
                    ...
                    for (let i = 1; i <= totalPeriods; i++) {
                        ...
                        if (endTime > currentTime) {
                            const startTime = moment(
                                new Date(year, (i - 1) * cycle, 1).getTime()
                            ).format("YYYY-MM-DD");
                    ...
                    if (item.executorId) {
                        if (!item.executeStartTime) {
                            this.$Message.warning(
                                `请选择第${periodsString}期执行开始时间`
                            );
                        } else if (!item.executeEndTime) {
                            this.$Message.warning(
                                `请选择第${periodsString}期执行结束时间`
                            );
                        } else {
                            const start = moment(
                                item.startTime,
                            ...
                            if (executeStart < start || executeStart > end) {
                                this.$Message.warning(
                                    `第${periodsString}期执行开始时间不在本周期时间段内`
                                );
                            } else if (executeEnd < start || executeEnd > end) {
                                this.$Message.warning(
                                    `第${periodsString}期执行结束时间不在本周期时间段内`
                                );
                            } else if (executeEnd < executeStart) {
                                this.$Message.warning(
                                    `第${periodsString}期执行结束时间不能小于执行结束时间`
                                );
                            } else {
                                valid = true;
                            }
            ...
            formDataSubmit() {
                this.$refs.taskForm.validate(valid => {
                    ...
                    if (formData.assetIds.length === 0) {
                        this.$Message.warning('请在保养科室对话框中选择设备');
                    } else if (valid) {
                        if (this.validPlainsDate(formData.pmTasks)) {
                            ...
                            this.submitLoading = true;
                            this.$axios
                                .post(`/maintenanceSchedule`, data)
                                .then(() => {
                                    this.$Message.success("新增保养计划成功");
                                    if (this.tempTask) {
                                        this.$router.push({
            ...
            //点击取消按钮
            cancelSubmit() {
                if (this.$route.name === "AllTwoTaskDetail") {
                    //
                    if (this.tempTask) {
                        this.$router.push({
                            name: "MaintenanceTask",
