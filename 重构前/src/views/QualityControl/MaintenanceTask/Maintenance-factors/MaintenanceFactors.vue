<template>
    <div class="inspection-element">
        <element-tabs :add-limit="addLimit" :edit-limit="editLimit" :typeListLoaded='typeListLoaded'
                      :del-limit="delLimit" :list-limit="listLimit" type-title="保养类别" item-title="保养项"
                      element-title="保养要素" :internal-type-list="internalTypeList" :custom-type-list="customTypeList"
                      :item-list="itemList" :element-list="elementList" @click-type="clickType" @click-item="clickItem"
                      @click-element="clickElement" @add-type="addType" @add-item="addItem" @add-element="addElement"
                      @del-type="delType" @del-item="delItem" @del-element="delElement"></element-tabs>
        <Modal v-model="modalState.show" :title="modalState.title" @on-visible-change="modalVisibleChange"
               class-name="vertical-center-modal">
            <i-input @on-keydown.enter="modalSubmit" ref="modalInput" v-model="modalState.input"
                     :placeholder="modalState.placeholder" class="kld-input"></i-input>
            <div slot="footer">
        ...
        <Modal v-model="modalState.addItemShow" :title="modalState.title" @on-visible-change="modalVisibleChange"
               class-name="vertical-center-modal" class="inspection-element-modal" width='1000'>

            <div class="convenient-switch">
                <span class="convenient-switch-title">快速添加保养项</span>
    ...
    computed: {
        ...
        created() {
            for (const {plPermission} of this.limit) {
                switch (plPermission) {
                    case "pm:maintain:element:add":
                        this.addLimit = true;
                        break;
                    case "pm:maintain:element:edit":
                        this.editLimit = true;
                        break;
                    case "pm:maintain:element:del":
                        this.delLimit = true;
                        break;
                    case "pm:maintain:element:list":
                        this.listLimit = true;
                        break;
                }
            }
        ...
        watch: {
            ...
            "modalState.type"(type) {
                switch (type) {
                    case "add-type":
                        this.modalState.title = "添加保养类别";
                        this.modalState.placeholder = "请输入保养类别名称";
                        this.modalState.validMessage = "保养类别名称不能为空";
                        this.modalState.successMessage = "添加保养类别成功";
                        this.modalState.freshField = "getTypeList";

                        this.axiosConfig = {
                            url: "/maintenanceOption/elementOption",
                            method: "POST"
                        };
                        break;
                    case "add-item":
                        this.modalState.title = "添加保养项";
                        this.modalState.placeholder = "请输入保养项名称";
                        this.modalState.validMessage = "保养项名称不能为空";
                        this.modalState.successMessage = "添加保养项成功";
                        this.modalState.freshField = "getItemList";

                        this.axiosConfig = {
                            url: "/maintenanceOption/elementType",
                            method: "POST"
                        };
                        break;
                    case "add-element":
                        this.modalState.title = "添加保养要素";
                        this.modalState.placeholder = "请输入保养要素名称";
                        this.modalState.validMessage = "保养要素名称不能为空";
                        this.modalState.successMessage = "添加保养要素成功";
                        this.modalState.freshField = "getElementList";

                        this.axiosConfig = {
                            url: "/maintenanceOption/element",
                            method: "POST"
                        };
                        break;
                    case "edit-element":
                        this.modalState.title = "编辑保养要素";
                        this.modalState.placeholder = "请输入保养要素名称";
                        this.modalState.validMessage = "保养要素名称不能为空";
                        this.modalState.successMessage = "编辑保养要素成功";
                        this.modalState.freshField = "getElementList";
        ...
        methods: {
            ...
            getTypeList() {
                return this.$axios
                    .post(
                        `/maintenanceOption/elementOption/${
                            this.user_in.osId
                            }/list`,
                        {
                            limit: 50,
            ...
            getItemList(type = this.currentType) {
                return this.$axios
                    .post(`/maintenanceOption/elementType/${type.id}/list`, {
                        limit: 50,
                        offset: 0,
                        params: {}
                    })
        ...
        getElementList(item = this.currentItem) {
            return this.$axios
                .post(`/maintenanceOption/element/${item.id}/list`, {
                    limit: 50,
                    offset: 0,
                    params: {}
                })
            ...
            delType(type) {
                kldConfirm({
                    ...
                    ok: () => {
                        this.$axios
                            .delete(`/maintenanceOption/elementOption/${type.id}`)
                            .then(() => {
                                this.$Message.success("删除保养类别成功");
                                this.getTypeList();
                            });
                    }
                });
            ...
            delItem(item) {
                kldConfirm({
                    ...
                    ok: () => {
                        this.$axios
                            .delete(
                                `/maintenanceOption/elementType/${
                                    this.user_in.osId
                                    }/${item.id}`
                            )
                            .then(() => {
                                this.$Message.success("删除保养项成功");
                                this.getItemList();
                            });
                    }
                });
            ...
            delElement(element) {
                kldConfirm({
                    ...
                    ok: () => {
                        this.$axios
                            .delete(`/maintenanceOption/element/${element.id}`)
                            .then(() => {
                                this.$Message.success("删除保养要素成功");
                                this.getElementList();
                            });
                    }
                });
            ...
            ItemModalSubmit() {
                ...
                this.$axios.post(`/maintenanceType/elementTypeAll`, requestBody)
                    .then(res => {
                        this.$Message.success(modalState.successMessage);
                        this[modalState.freshField]();
                        this.modalState.addItemShow = false;
